...
TEST(SolverTests, testSolve2Large) {
  ...
  // 5 * (75 - 1 + 100)
  std::vector<Element> expected{{Element(5), Element(75), Element(1),
                                 Element(MINUS), Element(100), Element(PLUS),
                                 Element(MULTIPLY)}};
  std::vector<Element> actual;
  EXPECT_TRUE(Solver::solve(input, target, actual));
  EXPECT_EQ(expected, actual);
...
TEST(SolverTests, testSolve4Large) {
  ...
  std::vector<Element> expected{
      {Element(7), Element(8), Element(100), Element(PLUS), Element(25),
       Element(MULTIPLY), Element(75), Element(DIVIDE), Element(MULTIPLY)}};
  std::vector<Element> actual;
  EXPECT_TRUE(Solver::solve(input, target, actual));
  EXPECT_EQ(expected, actual);
}
...
TEST(SolverTests, testSolveNoLarge) {
  ...
  // (1 + 2 + 10) * (1 + 7) * 8
  std::vector<Element> expected{{Element(1), Element(2), Element(10),
                                 Element(PLUS), Element(PLUS), Element(1),
                                 Element(7), Element(PLUS), Element(8),
                                 Element(MULTIPLY), Element(MULTIPLY)}};
  std::vector<Element> actual;
